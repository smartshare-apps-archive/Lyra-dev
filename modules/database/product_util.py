...
def formatCollectionConditions(collectionConditions):
    ...
    for condition_id, condition in collectionConditions.iteritems():
        conditionsString += condition["type"] + "<field_split>" + condition["rule"] + "<field_split>" + condition[
            "value"] + "<condition_split>"
        ...
        if product.has_key("resources"):
            if (product["resources"] is not None and product["resources"] != ""):
                imageResources = extractImageResources(
                    product["resources"])  # gets image resource id list from resource database
                imageURI = loadResourceURIList(imageResources, productDatabase)
                imageURI = formatImageURIs(imageURI)

                image_resources[product_id] = imageURI[
                    int(product["ImageSrc"])]  # so the product editor can access those image resources
        else:
